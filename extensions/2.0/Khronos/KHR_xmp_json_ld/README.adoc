...
This section describes how https://github.com/adobe/xmp-docs/tree/master/XMPNamespaces/XMPDataTypes[XMP data types (ISO 16684-1$6.3)] shall be encoded in glTF.
...
Additionally, the following are recommended:
  ...
  * For proper compatibility with https://www.iso.org/standard/79384.html[ISO 16684-3], an AboutURI should be included at the root level of each packet in an `@id`. For most purposes the value can be left blank.

[[lists-and-sets]]
=== Lists and Sets

The XMP specification makes a distinction between ordered lists and unordered sets, but in the case of JSON all arrays are considered to be ordered lists. (See https://www.rfc-editor.org/rfc/rfc7159.txt[RFC-7159].) In order to handle the differences between ordered lists and unordered sets, all arrays within a `packet` must be children of a `@list` or `@set` property to differentiate between ordered lists and unordered sets respectively. Descriptions for the two fields and examples are provided below.
