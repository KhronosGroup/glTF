...
This extension enables keeping domain specific data in relations with geometry.
...
The extension was originally developed for architecture, engineering and construction (AEC) industry to keep Building Information Model (*BIM*) data, but it's suitable for any other industry which operates huge amouts of information. We incourage any subject matter experts to discuss and improve this specification to create usefull general need metadata glTF extension.

### Objectives
Keep meta data of each real world object in the most compact and simple way possible.

### Assumptions
Each node with a mesh represents a real world object.
Nodes tree represents some logical hierarchy of real world objects, for a construction site with two buildings it might look like this:
```javascript
Model 1
Model 2
├Storey 1
...
Where:
Model - is a building,
Storey - includes all elements which lays or rise up starting from a particular level,
Category - groups elements by some characteristic (columns, furniture, ducts, etc.),
Element - leave `node` with `mesh` which represents a real construction product.

### Approach
A single piece of metadata for the node is represented by `property`. Property is a key-value pair, like 'name of the object's characteristic' - 'it's value'. To keep it simple name and value of the property are `strings`.
Node can also have a `type`. Type is a set of properties which is common for many nodes.
#### Example:
<em>There are two wooden doors in the building. Each is 900 mm (3') wide. First is 2,1 m (7') high, second is 2,4 m (8') high.
...
In this case each node points to the buffer where metadata for this node can be found. File is serialized 'GRIFFEL_bim_data' extension object with additional `nodeProperties` field. This field maps node properties and types to nodes by name.
```javascript
// gltf content:
"nodes": [
  {
