...
Each sequence of non-restart indices between primitive restart values **MUST** contain at least two indices. A vertex index **MAY** appear more than once in a line string, but adjacent indices **MUST** be distinct.

Each pair of adjacent non-restart indices in `lineStrings` **MUST** identify a hard edge of the triangle mesh primitive.

A hard edge encoded in `lineStrings` **MUST NOT** also be encoded in `visibility` or in any other entry of `lineStrings`. The same hard edge, regardless of direction, **MUST NOT** be identified more than once within `lineStrings`.
