...
To specify the geometry used to detect overlaps, we use the MSFT\_collision\_primitives extension. A node's `collider` property indexes into the document's top level array of collider objects; see the MSFT\_collision\_primitives extension for documentation on how these objects are used. In order to indicate that a `collider` should be used to generate a physics response on collision, that node should additionally have a `physicsMaterial` property, which indicates that it generates some physical response from interactions with other colliders. A collider attached to a node which does not contain a `physicsMaterial` should be interpreted as an object which cannot generate a collision response (i.e. no impulses are applied as a result of collision detection), but instead, should be used as a "trigger", which the application may utilize to generate callbacks for implementation of application-specific logic.

If the node is part of a rigid body (i.e., the node itself or an ascendant has `rigidBody` properties) then the collider belongs to that rigid body and should move with it during simulation. Otherwise the collider exists as a static object in the physics simulation which can be collided with but can not be moved.
