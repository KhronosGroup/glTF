...
```json
{
  "extensionsUsed": ["KHR_materials_pbrSpecularGlossiness", "VENDOR_physics"],
...
3. Names SHOULD be structured as `<PREFIX>_<scope>_<feature>`, where _scope_ is an existing glTF concept (e.g. mesh, texture, image) and _feature_ describes the functionality being added within that scope. This structure is recommended, but not required.
