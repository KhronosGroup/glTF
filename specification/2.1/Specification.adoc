...
Skinned mesh nodes with bounding volumes **SHOULD** be placed as children of the skeleton root node (if the skin defines a skeleton root node), or as a sibling of a root of its joint hierarchy. This allows the bounding volume to be transformed together with the skeletal hierarchy, such that root motion is applied to the bounding volume. This keeps the skinned mesh's bounding volume consistent with its skin-joint-node-determined visual representation.
...
| `"convexMesh"` | A convex mesh-based shape, relative to the local space, with normals facing outwards, away from the center of the convex shape.
...
| `"mesh"`       | A generic mesh-based shape, relative to the local space, with normals facing the same way as the mesh's face normals determined by the triangle winding order.
...
A sub-object with a key matching the shape type **MAY** be used to specify the size parameters of the shape. For the `mesh` and `convexMesh` shape types, the matching sub-object **MUST** be present. When the `type` of a shape refers to a built-in shape type, the shape must not supply a sub-object for a different shape type: for example, a shape whose `type` is "box" must not have a `sphere` property defined.
...
The `mesh` and `convexMesh` shape types are defined by a mesh, specified by a `mesh` index. The following conditions apply to meshes used by mesh-based shapes:
...
- Meshes used by `convexMesh` shapes **MUST** be pre-computed convex meshes with a non-zero volume.
- Meshes used by `convexMesh` shapes **MUST** have the triangles wound such that the normals face outwards from the center of the convex shape. The `NORMAL` attribute is ignored by convex shapes.
- Meshes used by `convexMesh` shapes **SHOULD** surround the local origin, such that the local origin is inside the convex shape, but this is not required.
...
.Implementation Note
====
Shapes of type `mesh` are usually concave. Prefer using `convexMesh` for convex mesh shapes. Physics engines usually have optimized paths for handling convex meshes, so using `mesh` for all mesh-based shapes may result in suboptimal performance.
====
