...
| `"convex"`   | A convex mesh-based shape, relative to the local space, with normals facing outwards, away from the center of the convex shape.
...
- Meshes used by convex shapes **MUST** be pre-computed convex meshes with a non-zero volume.
...
- Meshes used by convex shapes **SHOULD** surround the local origin, such that the local origin is inside the convex shape, but this is not required.
