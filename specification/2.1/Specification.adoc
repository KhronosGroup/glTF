...
A buffer **MAY** have its data provided by a GLB-stored chunk. This may be done explicitly by specifying the `buffer.chunk` property in the buffer object. When a buffer references a chunk by index, that chunk **MUST** be present in the GLB.

Alternatively, for compatibility with glTF 2.0, the first buffer in the `buffers` array **MAY** have its data implicitly provided by a GLB-stored `BIN` chunk when neither the `buffer.chunk` nor `buffer.uri` properties are defined. When such a buffer exists at buffer index 0, a `BIN` chunk **MUST** be present at chunk index 1 in the GLB file, immediately following the JSON chunk at chunk index 0. Other buffers do not have this implicit behavior, and **MUST** only have explicit references to its data via one of the `buffer.chunk` or `buffer.uri` properties.
...
In binary format version 2, the start and the end of each chunk **MUST** be aligned to a 4-byte boundary. In binary format version 3, the start of each chunk **MUST** be aligned to an 8-byte boundary. The start of each chunk is the next 4 or 8 byte aligned boundary after the end of the previous chunk. See chunks definitions for padding schemes. Chunks **MUST** appear in exactly the order given in <<table-chunktypes>>.
...
If the chunk is padded, either inside the binary chunk content, or outside the chunk length, the padding **MUST** be trailing zeros (`0x00`).
...
The value `0x00000000` indicates that the chunk data is plainly encoded, meaning that it is not compressed or encrypted. Other values indicate that the chunk data is encoded with a specific method, such as compressed and/or encrypted. Alternate encoding schemes **SHOULD** be defined in glTF extensions, however such extensions are not required to be marked as used in the JSON chunk.

Each chunk may use the same or a different encoding method compared to other chunks, as provided by the flexibility of this field being per-chunk. Additionally, since the file and chunk headers themselves are always encoded the same (never compressed or encrypted), this allows applications to read enough of the file to determine its overall layout even when the chunk contents cannot be read, ensuring that file identification utilities can still recognize the file as a Binary glTF asset.
