...
= glTF{tmtitle} 2.1 Specification
:tmtitle: pass:q,r[^™^]
:regtitle: pass:q,r[^®^]
...
glTF is an open interoperable 3D asset ‘transmission’ format that is compact, and efficient to process and render at runtime. glTF 2.1 is designed to be vendor- and runtime-neutral, usable by a wide variety of native and web-based engines and applications regardless of underlying platforms and 3D graphics APIs.
...
The following are outside the scope of glTF 2.1:

* **glTF is not a streaming format.** The binary data in glTF is inherently streamable, and the buffer design allows for fetching data incrementally, but there are no other streaming constructs in glTF 2.1.
...
glTF 2.1 assets **MAY** contain zero or more _scenes_, the set of visual objects to render. Scenes are defined in a `scenes` array. All nodes listed in `scene.nodes` array **MUST** be root nodes, i.e., they **MUST NOT** be listed in a `node.children` array of any node. The same root node **MAY** appear in multiple scenes.
...
Any glTF node **MAY** define a local space bounding volume using a node's `boundingVolume` property. A bounding volume is a simple geometric shape, optionally transformed, that **SHOULD** completely enclose the node's geometry.

Bounding volumes contain the following properties:

[options="header"]
|====
| Property      | Description
| `shape`       | The index of the shape in the top-level `shapes` array. Required.
| `rotation`    | The bounding volume's unit quaternion rotation relative to the node's local coordinate system.
| `scale`       | The bounding volume's scale along the rotated local X, Y, and Z axes.
| `translation` | The bounding volume's translation along the node's local X, Y, and Z axes.
|====

Multiple bounding volumes defined in a node hierarchy form a bounding volume hierarchy (BVH), which can be used to accelerate frustum culling, occlusion culling, ray tracing, and so on. Bounding volume hierarchies **SHOULD** have spatial coherence, meaning that a node's bounding volume completely encloses the geometry of its descendants.

Bounding volumes may be used to defer loading of content based on application-specific criteria, such as the angular size on a player's camera, or any other metric. Bounding volumes may be used to restrict rendering, interactions, or anything else to a specific volume of space. Implementations may choose to use bounding volumes for any purpose.

The tightness or looseness of the bounding volume may be customized by the asset author depending on the use case. For example, a bounding volume used for a skinned mesh is **RECOMMENDED** to be large enough to contain the mesh in all reasonable deformation states, such as the possible poses of the skinned geometry. Similarly, a bounding volume placed on a parent with multiple animated children is **RECOMMENDED** to be large enough to contain all of the children in all reasonable animation states. Alternatively, such a parent could simply not define a bounding volume, and dynamically rely on the bounding volumes of its children, if any.

[NOTE]
.Implementation Note
====
A bounding volume defined on a skinned mesh node is transformed by the mesh node's world transform (the composition of the node's local transform with all ancestor transforms), not by the skin's joint transforms. Because a skinned mesh node's own transform is ignored for the skinning computation, the mesh node's position in the scene hierarchy affects only the bounding volume, not the deformed geometry.

If an ancestor node's animated transform displaces the skinned geometry (for example, a translation animation targeting a joint or a navigation node), the skinned mesh node needs to be a descendant of that animated node for its bounding volume to track the displacement. A mesh node placed as a sibling of the displaced node will not inherit that displacement, and the bounding volume will remain stationary while the deformed mesh moves away from it. Animation applied to the root of the skinned joints therefore diverges from a bounding volume placed in parallel with the joint hierarchy: such a volume tracks the animation only when defined on the displaced node itself or on one of its descendants. See <<joint-hierarchy,Joint Hierarchy>>.

Node placement determines the bounding volume's world transform (translation, rotation, and scale inherited from ancestors); it does not account for skinned deformation. Bounding volumes are not skeleton-aware: a volume follows joint motion only through the transforms inherited from its node's ancestors. Per-frame tracking of skinned deformation (for example, recomputing bounds from posed joint transforms) would require runtime handling not defined by this specification. Skinned deformation may cause vertices to exceed any statically-defined volume regardless of the mesh node's placement.
====

`"box"`` and `"sphere"` are the supported shape types for bounding volumes due to their simplicity and efficiency. Additional shape types **MAY** be allowed via extensions.

Additionally, changing the shape parameters **SHOULD** be preferred over changing the bounding volume's `scale`.

[NOTE]
.Implementation Note
====
Many engines use axis-aligned bounding boxes (AABBs) for mesh culling. When converting from bounding volumes to AABBs programmatically, the resulting AABB should include all extents of the original bounding volume, including any transformations. Note that the transformations may be non-conformal (e.g. a transformation with a skew or a non-uniform scale).
====
...
A buffer **MAY** have its data provided by a GLB-stored chunk. This is done explicitly by specifying the `buffer.chunk` property in the buffer object. When a buffer references a chunk by index, that chunk **MUST** be present in the GLB.

Alternatively, for backwards compatibility with glTF 2.0, the first buffer in the `buffers` array **MAY** have its data implicitly provided by a GLB-stored `BIN` chunk when neither the `buffer.chunk` nor `buffer.uri` properties are defined. When such a buffer exists at buffer index 0, a `BIN` chunk **MUST** be present at chunk index 1 in the GLB file, immediately following the JSON chunk at chunk index 0. Other buffers do not have this implicit behavior, and **MUST** have one of its `buffer.chunk` or `buffer.uri` properties defined to explicitly reference its data.

Any glTF buffer with undefined `buffer.chunk` and `buffer.uri` properties that is not the first element of `buffers` array does not refer to the GLB-stored BIN chunk, and the behavior of such buffers is left undefined to accommodate future extensions and specification versions.

The byte length of the `BIN` chunk **MAY** be up to 7 bytes bigger than JSON-defined `buffer.byteLength` value to satisfy GLB padding requirements.
...
glTF 2.1 meshes support Linear Blend Skinning via skin objects, joint hierarchies, and designated vertex attributes.
...
The order of joints is defined by the `skin.joints` array and it **MUST** match the order of `inverseBindMatrices` accessor elements (when the latter is present). The `skeleton` property (if present) identifies the skeleton root of the skin: the common root of the skin's joints hierarchy or an ancestor thereof.

The skeleton root **SHOULD** be defined. When defined, it **SHOULD** be an ancestor of every node whose `skin` property references this skin, it **SHOULD** be the lowest common ancestor of the skin's joints and the nodes that reference the skin, and it **MAY** itself be a joint node.
...
glTF 2.1 separates texture access into three distinct types of objects: Textures, Images, and Samplers.
...
glTF 2.1 supports only static 2D textures.
...
View the full https://www.khronos.org/registry/glTF/specs/2.1/glTF-2.1.html#infinite-perspective-projection[infinite perspective projection].
====
endif::[]
...
====
This section contains formulas that may not display correctly in all preview tools.
View the full https://www.khronos.org/registry/glTF/specs/2.1/glTF-2.1.html#finite-perspective-projection[finite perspective projection].
====
endif::[]
...
====
This section contains formulas that may not display correctly in all preview tools.
View the full https://www.khronos.org/registry/glTF/specs/2.1/glTF-2.1.html#orthographic-projection[orthographic projection].
====
endif::[]
...
The core glTF 2.1 specification includes the following shape types:
...
* A 12-byte (binary format version 2) or 16-byte (binary format version 3) preamble, called the _header_.
...
Binary glTF is little endian. The figure below shows an example of the binary format version 2 layout:

.Binary glTF Version 2 Layout
image::figures/glb2.svg[pdfwidth=6in,align=left]

The figure below shows an example of the binary format version 3 layout:

.Binary glTF Version 3 Layout
image::figures/glb3.svg[pdfwidth=7in,align=left]
...
The file header consists of three entries, of different sizes depending on the binary format version.

For binary format version 2, the 12-byte header consists of three 4-byte entries:
...
For binary format version 3, the 16-byte header consists of two 4-byte entries and one 8-byte entry:
...
* `version` indicates the version of the Binary glTF container format. The glTF 2.1 specification defines version 2 and version 3.
+
...
+
In binary format version 3, the most significant bit **MUST** be 0, which limits file sizes to 8 EiB (exbibytes).
...
Each chunk has a header followed by a variable-length payload, with the header structure depending on the binary format version.

For binary format version 2, each chunk has an 8-byte header followed by a variable-length payload:
...
For binary format version 3, each chunk has a 16-byte header followed by a variable-length payload:

[source,c]
----
uint32 chunkType
uint32 chunkEncoding
uint64 chunkLength
ubyte[] chunkData
----

* `chunkType` indicates the type of chunk. See <<table-chunktypes>> for details.
* `chunkEncoding` indicates the encoding method used for the chunk data. See <<chunk-encoding>> for details.
* `chunkLength` is the length of `chunkData`, in bytes. In binary format version 3, the most significant bit **MUST** be 0, which limits chunk sizes to 8 EiB (exbibytes).
* `chunkData` is the binary payload of the chunk.

In binary format version 2, the start and the end of each chunk **MUST** be aligned to a 4-byte boundary. In binary format version 3, the start of each chunk **MUST** be aligned to an 8-byte boundary. The start of each chunk is the next 4-byte (v2) or 8-byte (v3) aligned boundary after the end of the previous chunk. See chunk definitions for padding schemes. Chunks **MUST** (v2) or **SHOULD** (v3) appear in exactly the order given in <<table-chunktypes>>.
...
| 1. | 0x4E4F534A | JSON  | Structured JSON content | 1 (v2), 1 or more (v3)
| 2. | 0x004E4942 | BIN\0 | Binary buffer           | 0 or 1 (v2), 0 or more (v3)
|====
...
In binary format version 2, this chunk **MUST** be the very first chunk of a Binary glTF asset. In binary format version 3, the first chunk that is a JSON chunk is the glTF JSON chunk, allowing for non-JSON chunks to precede the glTF JSON chunk. By reading this chunk first, an implementation is able to progressively retrieve resources from subsequent chunks. This way, it is also possible to read only a selected subset of resources from a Binary glTF asset.

If the chunk content is padded, the padding **MUST** use trailing `Space` chars (`0x20`). If the chunk has padding after the end of the content, the padding **MUST** use trailing zeros (`0x00`).
...
In binary format version 2, this chunk **MUST** be the second chunk of the Binary glTF asset. In binary format version 3, binary buffer chunks are explicitly referenced by index from the JSON chunk's buffers array, allowing for multiple binary buffer chunks to be present in a Binary glTF asset.

If the chunk is padded after the end of the content, the padding **MUST** be trailing zeros (`0x00`).
...
In binary format version 3 only, between each chunk's type and size, the `chunkEncoding` field indicates the encoding method used for the chunk data, such as compression or encryption.

The value `0x00000000` indicates that the chunk data is plainly encoded, meaning the chunk data does not need to be decoded (such as decompressed or decrypted) before use. Other values indicate that the chunk data requires decoding before use. Alternate encoding schemes **SHOULD** be defined in glTF extensions, however such extensions are not required to be marked as used in the JSON chunk.

Chunk-level encoding is distinct from any encoding that may be applied to data within chunks (such as storing a `.zip` file within a buffer in a `BIN` chunk).

Each chunk specifies its own encoding, which allows chunks within the same Binary glTF asset to use different encoding methods. This allows applications to read enough of the file to determine its overall layout even when the chunk contents cannot be read, ensuring that file identification utilities can still recognize the file as a Binary glTF asset.

Client implementations **MUST** refuse to load chunks with unknown encoding methods, and **MUST** refuse to load binary glTF files whose JSON chunk is encoded with an unknown method.

In binary format version 2, the `chunkEncoding` field is not present. All binary format version 2 chunks are plainly encoded, equivalent to `0x00000000` in binary format version 3.
...
View the full https://www.khronos.org/registry/glTF/specs/2.1/glTF-2.1.html#properties-reference[Properties Reference Section].
To suggest changes, open a Pull Request to edit the https://github.com/KhronosGroup/glTF/tree/main/specification/2.1/schema[JSON Schema Files].
====
endif::[]
...
====
This section is auto-generated, and will be a broken link when previewing.
View the full https://www.khronos.org/registry/glTF/specs/2.1/glTF-2.1.html#appendix-a-json-schema-reference[JSON Schema Reference].
To suggest changes, open a Pull Request to edit the https://github.com/KhronosGroup/glTF/tree/main/specification/2.1/schema[JSON Schema Files].
====
endif::[]
...
====
This section contains formulas that may not display correctly in all preview tools.
View the full https://www.khronos.org/registry/glTF/specs/2.1/glTF-2.1.html#specular-brdf[Specular BRDF Section].
====
endif::[]
...
====
This section contains formulas that may not display correctly in all preview tools.
View the full https://www.khronos.org/registry/glTF/specs/2.1/glTF-2.1.html#diffuse-brdf[Diffuse BRDF Section].
====
endif::[]
...
====
This section contains formulas that may not display correctly in all preview tools.
View the full https://www.khronos.org/registry/glTF/specs/2.1/glTF-2.1.html#fresnel[Fresnel Section].
====
endif::[]
...
View the full https://www.khronos.org/registry/glTF/specs/2.1/glTF-2.1.html#appendix-c-interpolation[Appendix C: Animation Sampler Interpolation Modes].
====
endif::[]
