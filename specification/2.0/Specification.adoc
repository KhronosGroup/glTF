...
A primitive object describes vertex attributes (data associated with each vertex), primitive topology (e.g., triangle set), vertex connectivity, and an optional material.
...
.Implementation Note
====
Splitting one mesh into several primitives can be useful to limit the number of vertices per draw call or to assign different materials to different parts of the mesh.
====
...
The following table provides attribute semantic names with their compatible accessor types and effective accessor component types for attribute semantics defined in this Specification. Future specification versions or extensions **MAY** define additional compatible types and/or effective component types for existing attribute semantics.

Accessors of the _uint32_ effective component type **MUST NOT** be used for any vertex attribute semantic, including application-specific semantics, unless explicitly enabled by future specification versions or extensions.
...
The `TEXCOORD_n`, `COLOR_n`, `JOINTS_n`, and `WEIGHTS_n` attribute semantic property names are stored in the form `[semantic]_[index]`, where `[index]` is a non-negative decimal integer, e.g., `TEXCOORD_0`, `TEXCOORD_1`, `COLOR_0`. Additionally, the following rules apply to the `[index]` syntax:

- the `[index]` values **MUST NOT** use leading zeroes for positive index values, e.g., `TEXCOORD_01` is invalid;
- the `[index]` values **MUST NOT** have more than nine digits.

Indexed attribute semantics **MUST NOT** have gaps, i.e., if an indexed attribute semantic with index `n` is present for any positive `n`, then the same indexed attribute semantic name with index `n-1` **MUST** also be present.
...
Among all possible indexed attribute semantic names, client implementations **SHOULD** support at least `TEXCOORD_0` and `TEXCOORD_1`, `COLOR_0`, and `JOINTS_0`/`WEIGHTS_0`.
...
If the primitive's `indices` property is not defined, the total number of vertices of the primitive is equal to the value of the `count` property of any attribute accessor, and the order of vertices is defined by the order of accessors' elements.

If the primitive's `indices` property is defined, the total number of vertices of the primitive is equal to the value of the `count` property of the indices accessor, and the order of vertices is defined by the values provided by the indices accessor. All values provided by the indices accessor **MUST** be less than the value of the `count` properties of the attribute accessors.

The accessor referenced by the `indices` property **MUST** have the `"SCALAR"` type and any of the _uint8_, _uint16_, or _uint32_ effective component types; the accessor **MUST NOT** provide the maximum possible value for its component type, i.e., 255 for _uint8_, 65535 for _uint16_, or 4294967295 for _uint32_.
...
The number of vertices of the primitive **MUST** be valid for the topology type used:

* For _points_, it **MUST** be positive.
* For _line loops_ and _line strips_, it **MUST** be greater than or equal to two.
* For _triangle strips_ and _triangle fans_, it **MUST** be greater than or equal to three.
* For _lines_, it **MUST** be positive and divisible by two.
* For _triangles_, it **MUST** be positive and divisible by three.
