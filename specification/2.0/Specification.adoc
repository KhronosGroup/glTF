...
Animation sampler interpolation modes define how to compute values of animated properties for the timestamps located between the keyframes. When the current (requested) timestamp exists in the animation data, its assosiated property value **MUST** be used as-is, without interpolation.
...
[latexmath]
++++
v_t = \frac{sin(a(1 - t))}{sin(a)} * {v}_k + s * \frac{sin(at)}{sin(a)} * v_{k+1}
++++

[NOTE]
.Implementation Note
====
Multiplication by the dot product's sign ensures that the spherical interpolation follows the short path.
====
...
For each timestamp stored in the animaition sampler, there are three assosiated keyframe values: in-tangent, property value, and out-tangent.
